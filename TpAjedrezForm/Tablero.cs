...
namespace TpAjedrezForm
{
    public partial class Tablero : Form
    {
        ...
        public Tablero(int cantTableros, Form menu, int inicio)
        {
            InitializeComponent();
        ...
        public static void fatalHorizontalVertical(int fila, int columna, int[,] tablero)
        {
            ...
            if (DentroTablero(fila, columna + j))
            {
                while (tablero[fila, columna + j] == 1 || tablero[fila, columna + j] == (int)Piezas.X)
                {
                    ...
                    _chessBoardPanels[fila, columna + j].BackgroundImage = Image.FromFile(path);
                    _chessBoardPanels[fila, columna + j].BackgroundImageLayout = ImageLayout.Stretch;
                    j++;
                    if (columna + j >= N)
                        break;
                }
            ...
            if (DentroTablero(fila + j, columna))
            {
                while (tablero[fila + j, columna] == 1 || tablero[fila + j, columna] == (int)Piezas.X)
                {
                    ...
                    _chessBoardPanels[fila+j, columna].BackgroundImage = Image.FromFile(path);
                    _chessBoardPanels[fila+j, columna].BackgroundImageLayout = ImageLayout.Stretch;
                    j++;
                    if (fila + j >= N)
                        break;
                }
        ...
        public static void fatalDiagonal(int fila, int columna, int[,] tablero)
        {
            ...
            if (DentroTablero(fila + k, columna + k)) //abajo a la derecha
            {
                while (tablero[fila + k, columna + k] == 1 || tablero[fila + k, columna + k] == (int)Piezas.X)
                {
                    ...
                    _chessBoardPanels[fila + k, columna + k].BackgroundImage = Image.FromFile(path);
                    _chessBoardPanels[fila + k, columna + k].BackgroundImageLayout = ImageLayout.Stretch;
                    k++;
        ...
        private void btn_proximoTablero_Click(object sender, EventArgs e)
        {
            ...
            {            
                ...
                while (true)
                {
                    ...
                    SetPosicion(Tablero.Piezas.Ra, fila, columna, TableroAux); //cuando implementemos el forms, esto queda determinado por el usuario
                    PosReina[0] = fila;
                    PosReina[1] = columna;
